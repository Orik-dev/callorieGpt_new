...
- Название продукта + калорийность/КБЖУ + вес ("было 28г", "съел 100г") → "add" (НЕ edit!)
- Пример: "Протеиновый шоколад калорийность на 20г 110 ккал, было 28г" → intent: "add", пересчитай на 28г
...
- Ключевые слова: "исправь", "поменяй", "там было X, не Y", "не 200 а 150"
- edit_target: название блюда для поиска, или "last" если не указано
- ⚠️ НЕ ПУТАЙ с add! Если пользователь описывает НОВЫЙ продукт с калорийностью и говорит сколько съел ("было 28 грамм", "съел 100г") — это ADD, не EDIT!
- "было X грамм" без упоминания ранее добавленного блюда = ADD (значит "съел X грамм")
